...
{
    [ExecuteInEditMode]
    public class MdxMapDoodads : MonoBehaviour
    {
        [SerializeField]
        ...
        public void Gen()
        {
            ...
            if (root == null) {
                root = new GameObject("Doodads").transform;
            ...
            for (int i = 0; i < doodads.Count; i++)
            {
                ...
                go.transform.position = doodad.doodad.location.ToU3d() * tileScale;
                go.transform.localEulerAngles = new Vector3(0, doodad.doodad.angle, 0);
                go.transform.localScale = doodad.doodad.scale.ToU3d() * tileScale;
