...
{
    public class MdxModelMulMeshImport
    {
        public GameObject gameObject { get; private set; }
        public Mesh mesh { get; private set; }
        private SortedDictionary<int, GameObject> meshGameObjects = new SortedDictionary<int, GameObject>();
        public List<Mesh> meshs { get; private set; }
        ...
        private void ImportMeshs()
        {
            if (cmodel.Geosets.Count == 0) return;
            ...
            for (int i = 0; i < cmodel.Geosets.Count; i++)
            {
                ...
                ImportMesh(cgeoset, i);
            }
        }


        private void ImportMesh(CGeoset cgeoset, int index)
        {
        ...
        private IEnumerator ImportTextures()
        {
            for (int i = 0; i < cmodel.Textures.Count; i++)
            {
                ...
                Debug.Log($"ImportTextures i:{i},  FileName={tex.FileName},path={MdxTextureManager.I.GetPath(tex)}, Current={it.Current}");
                ...
                Debug.Log($"ImportTextures i:{i}, ObjectId={tex.ObjectId}, ReplaceableId={tex.ReplaceableId}, FileName={tex.FileName},path={MdxTextureManager.I.GetPath(tex)}, texture={texture}");
        ...
        private void ImportMaterials(SkinnedMeshRenderer renderer)
        {
            ...
            for (int i = 0; i < cmodel.Materials.Count; i++)
            {
                ...
                for (int j = 0; j < cmaterial.Layers.Count; j++)
                {
                    ...
                    int textureReplaceableId = -1;
                    if (clayer?.Texture?.Object != null)
                    {
                        textureReplaceableId = clayer.Texture.Object.ReplaceableId;
